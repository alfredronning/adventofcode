...
def hueristic(pos, endpos, steps, backwards=False):
    if backwards:
        return pos[0] - endpos[0] + pos[1] - endpos[1] + steps
    return endpos[0] - pos[0] + endpos[1] - pos[1] + steps
...
def find_shortest(board, endpos, startpos, backwards):
    shortest = float("inf")
    board_by_steps = {}
    queue = [(startpos, board, 0, hueristic(startpos, endpos, 0, backwards))]
    checked = {(startpos, 0)}
    while queue:
        ...
        for move in [(0, 0), (1, 0), (-1, 0), (0, 1), (0, -1)]:
            ...
            h = hueristic(next_pos, endpos, current_steps, backwards)
    ...
    return shortest, board_by_steps[shortest]
...
if __name__ == "__main__":
    ...
    total = 0
    for i in range(3):
        shortest, board = find_shortest(board, endpos, startpos, i % 2)
        total += shortest
        tmpstart = startpos
        startpos = endpos
        endpos = tmpstart
