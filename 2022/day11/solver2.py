class Monkey:
    ...
    def inspect_items(self, root_mod):
        for item in self.items:
            item = self.apply_operation(item)%root_mod
            if item%self.test == 0:
    ...
    def __repr__(self):
        return "Monkey {}: {}".format(self.monkeyid, self.monkeybusiness)
...
if __name__ == "__main__":
    ...
    root_mod = 1
    for monkey in monkey_obj:
        root_mod *= monkey.test
    for r in range(10000):
        for monkey in monkey_obj:
            monkey.inspect_items(root_mod)
    res = 1
