...
def a_star(hills, start, ends, h):
    ...
    while open_set:
        current = open_set[0]
        #print_path(hills, came_from, current)
        if current in ends:
            i = 0
    ...
    for i, row in enumerate(hills):
        for j, height in enumerate(row):
            ...
            if height == "S" or height == "a":
                ends.append((i, j))
    h = lambda pos: min((end[0]-pos[0])**2+(end[1]-pos[1])**2 for end in ends)
    print(a_star(hills, start, ends, h))
