...
def search_solution_space(weps, armor, rings, piece, price, boss, cache):
    ...
    if piece > 0:
        ...
        if not wins(player, boss[:]) and price > cache["worst"]:
            cache["worst"] = price
            cache["worst_setup"] = [weps, armor, rings]
    worst = 0
    if piece == 0:
        ...
        worst = search_solution_space(weps, [], rings, piece+1, price, boss, cache)
