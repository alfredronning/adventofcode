...
def find_move_tile(unit, units, board, dead):
    ...
    for d in [(-1, 0), (0, -1), (0, 1), (1, 0)]:
        ...
        min_dist = float("inf")
        for other_unit in units:
            if unit == other_unit or other_unit in dead:
                continue
...
if __name__ == "__main__":
    ...
    if not alive_goblins:
        ...
        hp = sum(u.hp for u in alive_elfs)
        print("wins with remainding hitpoints " + str(hp))
        print("Outcome: "+ str(round*hp))
    if not alive_elfs:
        print("Goblins win after round: " + str(round))
        hp = sum(u.hp for u in alive_goblins)
        print("wins with remainding hitpoints " + str(hp))
        print("Outcome: "+ str(round*hp))
