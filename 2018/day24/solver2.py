...
class Group:
    def __init__(self, unit_type, id, init_text, boost):
        ...
        self.hp = int(grps[1])
        self.attack_dammage = int(grps[2+got_effect].split()[0]) + boost # type: ignore
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for i, grp in enumerate(immune.split("\n")[1:]):
            immune_grps.append(Group("immune", i+1, grp, boost))
        for i, grp in enumerate(infection.split("\n")[1:]):
            infection_grps.append(Group("infection", i+1, grp, 0))
        ...
        while immune_grps and infection_grps:
            ...
            total_units = sum(g.units for g in remainding_grps)
            ...
            if total_units == sum(g.units for g in remainding_grps):
                stalled = True
                break
            immune_grps = [g for g in immune_grps if g not in dead_grps]
            infection_grps = [g for g in infection_grps if g not in dead_grps]
        if immune_grps and not stalled:
            print(sum(g.units for g in immune_grps if g not in dead_grps))
            break
        boost += 1
