def simulate_fall(brick_map, occupied):
    ...
    while changed:
        ...
        for brick_idx in brick_map:
            ...
            new_bricks = []
            changed = True
            for brick in sorted(list(brick_map[brick_idx]), key=lambda x: x[2]):
    ...
    while changed:
        ...
        for brick_idx in brick_map:
            ...
            if any(b[2] == 1 for b in brick_map[brick_idx]):
                continue
            changed = True
            fallen.add(brick_idx)
            for brick in brick_map[brick_idx]:
                occupied.remove(brick)
    return len(fallen)
...
for brick_idx in brick_map:
    ...
    res += bricks_fall(brick_map, occupied_cp)
print(res)
