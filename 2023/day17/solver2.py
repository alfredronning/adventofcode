...
def search(board, start, end):
    ...
    while queue:
        ...
        for d in [(1, 0), (0, 1), (0, -1), (-1, 0)]:
            ...
            if neighbour_dir_count > 10:
                continue
            if neighbour_dir_count == 1 and dir_count < 4 and current != start:
                continue
            neighbour = current[0]+d[0], current[1]+d[1]
            if 0 <= neighbour[0] <= len(board) and 0 <= neighbour[1]>=0 and neighbour[0]<len(board) and neighbour[1]<len(board[0]):
